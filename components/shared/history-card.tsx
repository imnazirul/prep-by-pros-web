    ...
    >
      <div className="w-full">
        <div className="space-y-2">
          ...
          >
            <div className="space-y-1">
              <h3 className="line-clamp-1 text-2xl font-semibold text-white">
                {history.type === 'ORDER'
                  ? `Total ${history.items.length} Items`
                  : history.title}
              </h3>
              {history.type === 'PAYMENT' && (
                <p className="text-black-4 text-sm">
                  TxID: {history.payment_details.trx_id}{' '}
                </p>
              )}
            </div>
          ...
          {history.type === 'ORDER' && (
            <div className="flex items-center justify-between">
              ...
              <div className="grid shrink-0">
                <span className="text-black-4 text-base font-medium">
                  Delivered on
                </span>
          ...
          <div className="mt-5 grid grid-cols-[4.5fr_7.5fr] items-center gap-2 text-center">
            <Link href={`/order/${history.id}/refund`} className="block">
              <Button
